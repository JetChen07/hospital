...
<div class="container">
    ...
    <form class="form-horizontal" action="edit.do" method="post">
        <input name="id" class="form-control" type="hidden" value="${department.id}">
            ...
            <div class="col-sm-8">
                <select name="did" class="form-control">
                    <c:forEach items="${CLIST}" var="depclass">
                        <c:if test="${depclass.id==department.did}">
                            <option value="${depclass.id}" selected="selected">${depclass.name}</option>
                        </c:if>
                        <c:if test="${depclass.id!=department.did}">
